...
import { mockAuthApi, mockGamesApi, mockShopApi, mockTransactionsApi } from './mock/mockApi';
...
export const authApi = {
  async login(credentials: LoginRequest): Promise<AuthTokenResponse> {
    ...
    const response = await apiClient.post<AuthTokenResponse>(API_ENDPOINTS.AUTH.LOGIN, credentials);
    apiClient.setToken(response.token);
    return response;
  },
  ...
  async changePassword(data: ChangePasswordRequest): Promise<AuthTokenResponse> {
    ...
    const response = await apiClient.post<AuthTokenResponse>(API_ENDPOINTS.AUTH.CHANGE_PASSWORD, data);
    apiClient.setToken(response.token);
    return response;
  },
  ...
  async forgotPassword(username: string, email: string): Promise<void> {
    if (API_CONFIG.USE_MOCK) {
      return await mockAuthApi.forgotPassword({ username, contact_email: email });
    }
    await apiClient.post(API_ENDPOINTS.AUTH.FORGOT_PASSWORD, { username, contact_email: email });
  },

  async resetPassword(uid: string, token: string, newPassword: string): Promise<AuthTokenResponse> {
    if (API_CONFIG.USE_MOCK) {
      return await mockAuthApi.resetPassword({ uid, token, new_password: newPassword });
    }
  ...
  async enable2FA(otp: string): Promise<ShopUser> {
    ...
    return await apiClient.post<ShopUser>(API_ENDPOINTS.AUTH.ENABLE_2FA, { otp });
  },
...
export const gamesApi = {
  ...
  async getCartellaDraw(code: string, cartellaNumber: number): Promise<any> {
    ...
    return await apiClient.get<any>(API_ENDPOINTS.GAMES.CARTELLA_DRAW(code, cartellaNumber));
  },
};
...
export const shopApi = {
  ...
  async updateProfile(data: ProfileUpdateRequest): Promise<ShopProfile> {
    ...
    return await apiClient.put<ShopProfile>(API_ENDPOINTS.SHOP.UPDATE_PROFILE, data);
  },
};
...
export const transactionsApi = {
  async getHistory(): Promise<Transaction[]> {
    ...
    return await apiClient.get<Transaction[]>(API_ENDPOINTS.TRANSACTIONS.HISTORY);
  },
  ...
  async deposit(amount: string, reference: string): Promise<Transaction> {
    ...
    return await apiClient.post<Transaction>(API_ENDPOINTS.TRANSACTIONS.DEPOSIT, { amount, reference });
  },
  ...
  async withdraw(amount: string, reference: string): Promise<Transaction> {
    ...
    return await apiClient.post<Transaction>(API_ENDPOINTS.TRANSACTIONS.WITHDRAW, { amount, reference });
  },
};
