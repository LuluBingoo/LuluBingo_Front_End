...
export const Playground: React.FC<PlaygroundProps> = ({ 
  ...
  const handleDeclareWinner = (cartelaNumber: string) => {
    alert(`🎉 Cartela ${cartelaNumber} declared as WINNER!\n\nGame will be stopped.`);
    // In real app, this would call the API to complete the game
    stopGame();
  };
  ...
  const openCartelaModal = () => {
    if (activeCartelas.length === 0) {
      alert('No cartelas in this game');
      return;
    }
    setSelectedCartela('');
    setShowCartelaModal(true);
  ...
  const checkCartela = () => {
    ...
    if (!isValid) {
      alert(`Cartela ${cartelaNumber} not in game`);
      return;
    }
    
    // Simple check - just show if cartela exists
    alert(`Cartela ${cartelaNumber} is valid and active in this game.`);
  };
